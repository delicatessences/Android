...
public class LastRecipeCursorLoader<D> extends AsyncTaskLoader<Cursor> {
    ...
    public Cursor loadInBackground() {
        ...
        if (mActivity instanceof OrmLiteBaseActionBarActivity) {
            try {
                ...
                QueryBuilder<Recipe, Integer> queryBuilder = dao.queryBuilder();
                queryBuilder.orderBy(Recipe.CREATION_DATE_FIELD_NAME, false);
                queryBuilder.limit(3L);
                PreparedQuery<Recipe> preparedQuery = queryBuilder.prepare();
                CloseableIterator<Recipe> iterator = dao.iterator(preparedQuery);
...
// An async query came in while the loader is stopped
    ...
    public LastRecipeCursorLoader(Context context) {
        super(context);
        mActivity = context;
