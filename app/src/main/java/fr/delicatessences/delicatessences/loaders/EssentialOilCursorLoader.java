...
public class EssentialOilCursorLoader extends AsyncTaskLoader<Cursor> {
    ...
    public Cursor loadInBackground() {
        ...
        if (mActivity instanceof OrmLiteBaseActionBarActivity) {
            try {
                ...
                if (mOnlyFavorites) {
                    queryBuilder.where().eq(EssentialOil.FAVORITE_FIELD_NAME, true);
                }
                queryBuilder.orderBy(EssentialOil.NAME_FIELD_NAME, true);
                PreparedQuery<EssentialOil> preparedQuery = queryBuilder.prepare();
                CloseableIterator<EssentialOil> iterator = dao.iterator(preparedQuery);
...
// An async query came in while the loader is stopped
    ...
    public EssentialOilCursorLoader(Context context, boolean onlyFavorites) {
        super(context);
        mActivity = context;
